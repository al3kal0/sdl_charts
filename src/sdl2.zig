...
// pub const RenderDrawLines = c.SDL_RenderDrawLines;
// pub const RenderDrawPoints = c.SDL_RenderDrawPoints;
...
{
    return if(c.SDL_PollEvent(event) != 0) true else false;
}

/// Create a window with the specified position, dimensions, and flags.
pub inline fn CreateWindow(title: []const u8,
                           x: u32,
                           y: u32,
                           w: u32,
                           h: u32,
...
{
    if(c.SDL_SetWindowFullscreen(window, @enumToInt(flags)) != 0) return Error;
}
...
pub inline fn RenderGeometry(renderer: Renderer,
                             texture: Texture,
                             vertices: []Vertex,
                             indices: ?[]u16
                             ) !void
...
{
    if(c.SDL_RenderDrawLines(renderer,
                             @ptrCast([*c]Point, points),
                             points.len) != 0) return Error;
}

/// Draw multiple points on the current rendering target.
pub inline fn RenderDrawPoints(renderer: Renderer, points: []Point) !void
{
    if(c.SDL_RenderDrawPoints(renderer,
                              @ptrCast([*c]Point, points),
                              points.len) != 0) return Error;
}
...
{
    if(c.SDL_RenderDrawRects(renderer, @ptrCast([*c]Rect, rects), rects.len) != 0) return Error;
}

/// Fill some number of rectangles on the current rendering target with the drawing color.
pub inline fn RenderFillRects(renderer: Renderer, rects: []Rect) !void
{
    if(c.SDL_RenderFillRects(renderer, @ptrCast([*c]Rect, rects), rects.len) != 0) return Error;
}

/// Set the color used for drawing operations (Rect, Line and Clear).
pub inline fn SetRenderDrawColor(renderer: Renderer, rgba: Color) !void
{
    if(c.SDL_SetRenderDrawColor(renderer, rgba.r, rgba.g, rgba.b, rgba.a) != 0) return Error;
}
...
{
    if(c.SDL_RenderClear(renderer) != 0) return Error;
}
